...
const koishi_1 = require("koishi"); // 引入必要的模块和函数
// 可选的模型列表
exports.model = [
    "wd14-vit.v1",
    "wd14-vit.v2",
...
exports.Config = koishi_1.Schema.object({
    ...
    filemod: koishi_1.Schema.boolean().description('是否启用文件读取(需要gohttp支持)').default(false),
    onebotapi: koishi_1.Schema.string().description('onebot API 服务器地址').default('http://127.0.0.1:8888'),
    ...
    ifded: koishi_1.Schema.boolean().description('是否去除重复项').default(false),
});
// 应用插件功能
    ...
    ctx.command('反推')
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                ...
                if (tagsObject) {
                    const sortedTags = Object.entries(tagsObject).sort((a, b) => b[1] - a[1]);
                    const sortedTagNames = sortedTags.map(([tag, _]) => tag.replace(/_/g, ' '));
                    let resultString;
                    if (config.ifded) {
                        // 去除重复项
                        const filteredTags = sortedTagNames.filter(tag => !sortedTagNames.some(otherTag => otherTag !== tag && otherTag.includes(tag)));
...
async function trans_img_to_base64(imgurl, ctx) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            return (0, koishi_1.arrayBufferToBase64)(response.data);
        }
        catch (error) {
